...
export default function GoogleMapComponent({
	...
	const [selectedRecordData, setSelectedRecordData] =
	...
	useEffect(() => {
		if (selectedRecord && isLoaded) {
			const record = validRecords.find(
				(record, index) => getRecordId(record, index) === selectedRecord
			);
			if (record) {
	...
	const handleMarkerDragEnd = (
		...
		const updatedRecord = {
			...
			georeference: {
				...
				source: record?.metadata?.georeference?.source
					? `${record?.metadata?.georeference?.source} (adjusted)`
					: "manually adjusted",
			},
		};
		console.log(
		...
		if (
			selectedRecordData &&
			getRecordId(record, index) === selectedRecord
		) {
			...
			<div className="h-full min-h-[500px] flex flex-col">
					...
					<div className="flex items-start">
						...
						<div>
							...
							<p className="text-sm text-amber-700 mt-1">
								The Google Maps API key is invalid or has
								restrictions. Please check your API key
								configuration.
							</p>
							<div className="mt-3">
								<a
				...
				<div className="flex-1 bg-gray-100 p-4 overflow-auto">
					<h3 className="font-medium mb-3">
						Asset Locations ({validRecords.length})
					</h3>
						...
						{validRecords.map((record, index) => {
							...
							return (
								<Card
									...
									onClick={() =>
										handleMarkerClick(record, index)
									}>
									<div className="flex items-start">
										<div
				...
				{validRecords.map((record, index) => {
					...
					return (
						<Marker
							...
							icon={
								createMarkerIcon(record, isSelected) ??
								undefined
							}
							...
							animation={
								...
								typeof window !== "undefined" &&
								window.google
									? (window as any).google.maps.Animation
											.BOUNCE
									: undefined
							}
							...
							draggable={true}
							onDragEnd={(e) =>
								handleMarkerDragEnd(e, record, index)
							}
						/>
					);
				})}
